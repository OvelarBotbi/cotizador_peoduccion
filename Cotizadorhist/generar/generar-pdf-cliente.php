...
$idunidad = 0;
$unidadn = $conn->query("SELECT * FROM histrotatorio as hist left JOIN imagenes as img ON img.id_comp=hist.plataforma WHERE hist.numcotizacion='$numcotizacion'");
if ($verr = $unidadn->fetch_array(MYSQLI_ASSOC)) {
  $idunidad = $verr['unidad'];
  $nomrotatorio = $verr['nombre'];
  $subtotal = $verr['sub'];
  $descuento = $verr['descuento'];
  $iva = $verr['IVA'];
  $totalSis = $verr['precio'];
  $gastos = $verr['ginstalacion'];
  $total90 = $verr['total'];
  if ($verr['dir_img'] != "" && $verr['dir_img'] != null) {
    $imagen = $verr['dir_img'];
    $imagen = '../../' . $imagen;
  }
}
...
$result = $conn->query("SELECT * FROM histrotatorio WHERE numcotizacion='$numcotizacion'");
...
<page backtop="-3.000mm" backbottom="-3.000mm" backleft="-3.000mm" backright="-3.000mm">
  <div style="height: 85%; width: 100%; background-color: #00528A;">
    ...
    <div style="height: 50%; width: 100%; display: inline-block; position: absolute; top:0px; left:0px;">
      <!-- espacio de imagen -->
      <div style="background-color: white; height: 90%; margin-left: 4%; margin-right; 4%; margin-top: 4%; width: 92%;">
        <!--Cuadro blanco -->
        <!-- Cuadrito azul -->
        <img src="<?php echo $imagen; ?>" style="width: 100%; height: 100%; object-fit: contain; margin-bottom: 4%;" />
        <div style="display: block; position: absolute; top: 15%; right: 0; background-color: #00528A;width: 25%; height: 5%; padding: 10px; padding-top: 0px;">
      ...
      <div style="height: 45%; width: 100%; display: inline-block; position: relative;">
        ...
        <div style="background-color: white; display: block; width: 30%; height: 40%; position: absolute; left: 4%; top: -10%; padding-left: 4mm;">
          ...
          <h2 style="color: #929295; padding: 15px; font-size: 14px; text-align: left;"><img src="../public/static/here_icon.png" style="height: 15px; width: 15px;" />
            J.F. Brittingham 110 nte.<br /> Int. 1, Cd. Industrial Trc.<br />Torreón, Coah. México.<br />
          </h2>
          <h2 style="color: #929295; padding: 15px; font-size: 14px; font-weigth: 200; text-align: left;"><img src="../public/static/telefono_icon.png" style="height: 15px; width: 15px;">
            (871) 747 1300 / 05
          </h2>
          <br />
          <br />
        ...
        <div style="background-color: white; display: block; width: 56%; text-align: left;position: absolute; right: 4%; top: -30%; padding: 10px;">
          <div>
            <img class="imagen-left" src="../../img/logo.png" />
          </div>
          <div>
...
<?php
$idunidad = 0;
$unidad = $conn->query("SELECT * FROM histrotatorio WHERE numcotizacion='$numcotizacion'");
...
$result = $conn->query("SELECT * FROM hist_cotizacion_detallada WHERE numcotizacion='$numcotizacion' and id_component='$idunidad' and ischeck=1 ORDER BY orden ");
//$reg= $result->num_rows;
if ($s_descripcion == 1) {
  while ($var = mysqli_fetch_row($result)) {
    ...
    $idunidad = $var[1];
    $idcomponente = $var[1];
    $hijos = $conn->query("SELECT * FROM hist_cotizacion_detallada WHERE numcotizacion='$numcotizacion' and id_component='$idunidad' and ischeck=1 ORDER BY orden");
    ...
    print "<page backtop='-3mm' backbottom='-19mm' backleft='-3mm' backright='-3mm'>
        ...
        <tbody>
        ";
    $resul = $conn->query("SELECT * FROM hist_cotizacion_detallada WHERE numcotizacion='$numcotizacion'  and id_component='$idunidad' and ischeck=1 ORDER BY orden");
  ...
  $result = $conn->query("SELECT * FROM hist_cotizacion_detallada WHERE numcotizacion='$numcotizacion'  and id_component=id_producto and ischeck=1 ORDER BY orden");
  //$reg= $result->num_rows;
  while ($var = mysqli_fetch_row($result)) {
    $nombreComp = utf8_encode($var[2]);
    $cantidadcomp = $var[3];
    ...
    print "<page backtop='-3mm' backbottom='-19mm' backleft='-3mm' backright='-3mm'>
        ...
        <tbody>
        ";
    $resul = $conn->query("SELECT * FROM hist_cotizacion_detallada WHERE numcotizacion='$numcotizacion' AND id_component='$idcomponente'  and ischeck=1 ORDER BY orden");
    $numreg = $resul->num_rows;
    $i = 1;
    $j = 0;
    $cadena = "";
    ...
    if ($numreg > 17 && ($reggop > 0 || $j == 1)) {
      ...
      <draw class='main-draw'>
          <polygon id='left-polygon'  points='0 230,269 230,327 0,0 0'>
          <polygon id='right-polygon' points='900 150,216 150,242 0,900 0'>
      </draw><br>
      <div class='header-title'>
        <h3 id='title-desc'>$nombreComp ($cantidadcomp) </h3>
      </div>
...
//-------------------------------   PDF SECUNDARIO  ----------------------------------------------------
  ...
  //------------------Plataforma-----------------------------
  $hijos = $conn->query("SELECT * FROM hist_cotizacion_detallada WHERE numcotizacion='$numcotizacion' AND id_component=$idcomponente and ischeck=1 ORDER BY orden");
  $busca_plataforma =  $conn->query("SELECT * FROM hist_cotizacion_detallada as cot left JOIN imagenes as img ON img.id_comp=cot.id_producto where cot.id_usuario='$idUsuario' and cot.numcotizacion='$numcotizacion'  and cot.id_component='$idcomponentepadre'  and cot.descripcion LIKE '%PLATAFORMA%' and ischeck=1 ORDER BY orden");
  ...
  if ($numreg > 0) {
    ...
    $pag = 0;
    $busca_plataforma2 =  $conn->query("SELECT * FROM hist_cotizacion_detallada as cot left JOIN imagenes as img ON img.id_comp=cot.id_producto where cot.id_usuario='$idUsuario' and cot.numcotizacion='$numcotizacion' and cot.id_component='$idcomponentepadre'  and cot.descripcion LIKE '%PLATAFORMA%' and ischeck=1 ORDER BY orden");
  ...
  if ($numreg > 0) {
    ...
    $pag = 0;
    $busca_ordeno2 =  $conn->query("SELECT * FROM hist_cotizacion_detallada as cot left JOIN imagenes as img ON img.id_comp=cot.id_producto where cot.id_usuario='$idUsuario' and cot.numcotizacion='$numcotizacion' and cot.id_component='$idcomponentepadre' and (cot.descripcion LIKE '%UNIDADES DE ORDE%' or cot.descripcion LIKE '%GLANDULA%' or cot.descripcion LIKE '%BOMBAS DE LECHE%' or cot.descripcion LIKE '%GRUPOS DE RECIBO%' or cot.descripcion LIKE '%GRUPO DE RECIBO%' or cot.descripcion LIKE '%LINEAS DE LECHE%' or cot.descripcion LIKE '%SISTEMA DE PULSACION%' or cot.descripcion LIKE '%TUBERIA DE PVC PULSACION%' or cot.descripcion LIKE '%SOPORTERIA%' or cot.descripcion LIKE '%FILTROS DE LECHE%' or cot.descripcion LIKE '%SISTEMA DE VACIO%' or cot.descripcion LIKE '%CONTROLADOR DE VACIO DE RESPALDO%' or cot.descripcion LIKE '%SISTEMA DE LAVADO%' or cot.descripcion LIKE '%ARREADOR%' or cot.descripcion LIKE '%ARRASTRE DE LECHE%' or cot.descripcion LIKE '%UNIDAD MOTRIZ DE EMERGENCIA%' or cot.descripcion LIKE '%FILTROS A TANQUE%' or cot.descripcion LIKE '%SUCCION DE LAVADO%') and ischeck=1 ORDER BY cot.orden;");
  ...
  //------------------------------------------opcionales------------------------------------
  $busca_opcionales = $conn->query("SELECT * FROM hist_cotizacion_detallada as cot LEFT JOIN imagenes as img ON img.id_comp=cot.id_producto WHERE cot.id_usuario='$idUsuario' and cot.numcotizacion='$numcotizacion' and cot.id_component=cot.id_producto ORDER BY cot.descripcion ");
  ...
  if ($numreg > 0) {
    ...
    $pag = 0;
    $busca_opcionales2 =  $conn->query("SELECT * FROM hist_cotizacion_detallada as cot LEFT JOIN imagenes as img ON img.id_comp=cot.id_producto WHERE cot.id_usuario='$idUsuario' and cot.numcotizacion='$numcotizacion' and cot.id_component=cot.id_producto ORDER BY cot.descripcion ");
