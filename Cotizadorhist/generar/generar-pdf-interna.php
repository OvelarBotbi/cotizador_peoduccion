...
$idunidad=0;
$s_descuento=0;
   $unidadn=$conn->query("SELECT * FROM histrotatorio WHERE numcotizacion='$numcotizacion'");
  ...
  $result=$conn->query("SELECT * FROM histrotatorio as hist left JOIN imagenes as img ON img.id_comp=hist.plataforma WHERE hist.numcotizacion='$numcotizacion'");
...
<?php
 ...
 $descuento2=($descuento2/100)*$totalSis;
...
$notas= multiexplode(array("-","*"), $notaa);
 print"<page backtop='-3mm' backbottom='-19mm' backleft='-3mm' backright='-3mm'>
  ...
  <div class='container' style='padding-left:10px; padding-right:10px;'>
    <p class='totales' style:>";
    if($s_descuento==1){
      print"
        <h4>Total Sistema: $$totalSis</h4>
        <h4>Descuento Inicial: $$descuento</h4>
        <h4>Descuento Negociable: $$descuento2</h4>
...
<?php
  ...
  try
  {
      ...
      $html2pdf->Output('CotInterna '.$nomcliente.Date("d-m-Y",time()).'.pdf');
  }
